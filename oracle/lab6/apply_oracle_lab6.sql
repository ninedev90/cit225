...
,(SELECT   common_lookup_lab_id
  ...
  WHERE    common_lookup_lab_context = 'ITEM_LAB'
  AND      common_lookup_lab_type = 'DVD_WIDE_SCREEN')
...
-- ----------------------------------------------------------------------
-- Step #3c - [26 points] Insert a new member_lab account with three contact_labs.
-- ----------------------------------------------------------------------
...
,(SELECT   common_lookup_lab_id
  FROM     common_lookup_lab
  WHERE    common_lookup_lab_type = 'HOME')           -- address_lab_type
,'Provo'                                       -- city
,'Utah'                                             -- state_province
,'84604'                                          -- postal_code
...
,'900 E, 300 N'                              -- street_address_lab
,(SELECT   system_user_id
  FROM     system_user
  WHERE    system_user_name = 'SYSADMIN')         -- created_by
, SYSDATE                                         -- creation_date
...
,(SELECT   common_lookup_lab_id
  ...
  AND      common_lookup_lab_type = 'HOME')           -- telephone_lab_type
,'001'                                            -- country_code
,'801'                                            -- area_code
,'333-3333'                                       -- telephone_lab_number
,(SELECT   system_user_id
  FROM     system_user
  WHERE    system_user_name = 'SYSADMIN')         -- created_by
, SYSDATE                                         -- creation_date
...
COLUMN street_address_lab  FORMAT A14  HEADING "Street Address"
...
COLUMN rental_lab_id   FORMAT 9999
COLUMN rental_lab_days FORMAT A14
COLUMN rental_labs     FORMAT 9999
COLUMN item_labs       FORMAT 9999
SELECT   c.last_name||', '||c.first_name||' '||c.middle_name AS full_name
,        r.rental_lab_id
,       (r.return_date - r.check_out_date) || '-DAY RENTAL' AS rental_lab_days
,        COUNT(DISTINCT r.rental_lab_id) AS rental_labs
,        COUNT(ri.rental_item_lab_id) AS item_labs
FROM     rental_lab r INNER JOIN rental_item_lab ri
ON       r.rental_lab_id = ri.rental_lab_id INNER JOIN contact_lab c
...
ALTER TABLE common_lookup_lab
ADD (common_lookup_lab_column    VARCHAR2(30));

ALTER TABLE common_lookup_lab
ADD (common_lookup_lab_code      VARCHAR2(30));
...
UPDATE   common_lookup_lab
SET      common_lookup_lab_column = common_lookup_lab_table || '_TYPE'
WHERE    common_lookup_lab_table IN
          (SELECT table_name
           FROM   user_tables);
...
-- ----------------------------------------------------------------------
COLUMN common_lookup_lab_context  FORMAT A14  HEADING "Common|Lookup Context"
COLUMN common_lookup_lab_table    FORMAT A12  HEADING "Common|Lookup Table"
COLUMN common_lookup_lab_column   FORMAT A18  HEADING "Common|Lookup Column"
COLUMN common_lookup_lab_type     FORMAT A18  HEADING "Common|Lookup Type"
SELECT   common_lookup_lab_context
...
-- ----------------------------------------------------------------------
-- remove constranint not null of common_lookup_lab_context
ALTER TABLE common_lookup_lab
DROP CONSTRAINT nn_clookup_lab_1;
...
ALTER TABLE common_lookup_lab
MODIFY common_lookup_lab_table VARCHAR2(30) NOT NULL;

ALTER TABLE common_lookup_lab
MODIFY common_lookup_lab_column VARCHAR2(30) NOT NULL;

CREATE UNIQUE INDEX unique_month ON common_lookup_lab ( 
 common_lookup_lab_table, 
 common_lookup_lab_column, 
 common_lookup_lab_type 
);
...
-- ----------------------------------------------------------------------
COLUMN common_lookup_lab_table  FORMAT A14 HEADING "Common|Lookup Table"
COLUMN common_lookup_lab_column FORMAT A14 HEADING "Common|Lookup Column"
COLUMN common_lookup_lab_type   FORMAT A8  HEADING "Common|Lookup|Type"
...
SELECT   cl.common_lookup_lab_table
,        cl.common_lookup_lab_column
,        cl.common_lookup_lab_type
,        COUNT(a.address_lab_id) AS count_dependent
,        COUNT(DISTINCT cl.common_lookup_lab_table) AS count_lookup
FROM     address_lab a RIGHT JOIN common_lookup_lab cl
ON       a.address_lab_type = cl.common_lookup_lab_id
WHERE    cl.common_lookup_lab_table = 'ADDRESS'
AND      cl.common_lookup_lab_column = 'ADDRESS_TYPE'
AND      cl.common_lookup_lab_type IN ('HOME','WORK')
...
,        cl.common_lookup_lab_column
,        cl.common_lookup_lab_type
,        COUNT(t.telephone_lab_id) AS count_dependent
,        COUNT(DISTINCT cl.common_lookup_lab_table) AS count_lookup
FROM     telephone_lab t RIGHT JOIN common_lookup_lab cl
ON       t.telephone_lab_type = cl.common_lookup_lab_id
WHERE    cl.common_lookup_lab_table = 'TELEPHONE'
AND      cl.common_lookup_lab_column = 'TELEPHONE_TYPE'
AND      cl.common_lookup_lab_type IN ('HOME','WORK')
GROUP BY cl.common_lookup_lab_table
,        cl.common_lookup_lab_column
,        cl.common_lookup_lab_type;
